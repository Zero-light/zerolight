...
- **日志** — 8 篇设计随笔：留白、深色界面、微交互、光的隐喻（支持按标签筛选）
...
npm run build    # 生产构建（全静态导出 29 个 URL）
npm start        # 本地生产预览
```

## 部署

推送到 `main` 分支即触发 Vercel 自动部署。SEO 已配置：sitemap.xml（29 URL）、robots.txt、RSS feed（8 篇全文）、OpenGraph 图、PWA manifest。
