...
- **日志** — 13 篇设计随笔：留白、深色界面、微交互、光的隐喻（支持按标签筛选）
- **词汇表** — 12 个词构成我们的设计语言，与随笔双向关联
- **本站** — 设计哲学的自我审查：零位图、零依赖、系统字体、全静态
...
npm run build    # 生产构建（全静态导出 37 个 URL）
npm start        # 本地生产预览
```

## 部署

推送到 `main` 分支即触发 Vercel 自动部署。SEO 已配置：sitemap.xml（37 URL）、robots.txt、RSS feed（13 篇全文）、OpenGraph 图、PWA manifest。
